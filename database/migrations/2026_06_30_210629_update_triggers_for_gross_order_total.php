...
return new class extends Migration
{
    ...
    public function up(): void
    {
              ...
              SET p.valor_total =
                  (SELECT COALESCE(SUM(ip.valor_total),0)
                    ...
                    WHERE ip.pedido_id = NEW.pedido_id
                      AND ip.status_item IN ('ativo', 'devolvido'))
        ...
        // 5. Recalcular valores totais dos pedidos existentes para incluir itens devolvidos
        ...
        foreach ($pedidos as $p) {
            ...
            $newTotal = $sum + (float)$p->valor_frete - (float)$p->valor_desconto;
            
            if (abs($newTotal - (float)$p->valor_total) > 0.001) {
    ...
    public function down(): void
    {
        ...
        // 5. Recalcular valores totais dos pedidos existentes para excluir itens devolvidos
        $pedidos = DB::table('pedidos')->get();
        foreach ($pedidos as $p) {
            $sum = DB::table('items_pedido')
                ->where('pedido_id', $p->id)
