...
*   **Transferências entre Contas (Vínculo Directo / Unilateral):** Quando ocorrem transferências entre contas bancárias da empresa (ex: Pix enviado do Mercado Pago para a conta do Banco Inter), nem sempre ambos os extratos possuem a transação importada ao mesmo tempo. O módulo de conciliação suporta:
    1. **Vínculo entre duas transações existentes no extrato** (1 saída + 1 entrada).
    2. **Transferência Direta (Unilateral / Contrapartida):** Quando apenas um dos lados está no extrato (ex: a entrada no Banco Inter ou a saída no Mercado Pago), o operador pode selecionar a conta bancária de contrapartida. O sistema gera automaticamente o lançamento de contrapartida na conta destino/origem e concilia a transação com 1 clique.
