...
$pedidos = DB::table('pedidos')
    ...
    ->select(DB::raw('COUNT(*) as total_pedidos'), DB::raw('SUM(total) as faturamento_total'), DB::raw('AVG(total) as ticket_medio_pedido'))
    ->first();
...
// Ticket Médio por Pedido nos últimos 30 dias / 90 dias / 2026
...
// 3. Ticket Médio por SACOLINHA DE CLIENTE EM LIVES RECENTES (Sacolinhas por cliente)
...
echo "--- 3. TICKET MÉDIO POR SACOLINHA / COMPRADOR EM LIVES ---\n";
echo "Total de Sacolinhas de Clientes Auditadas: " . number_format($totalSacolasLive, 0, ',', '.') . "\n";
echo "Ticket Médio por Sacolinha (por Cliente por Live): R$ " . number_format($ticketMedioSacolaLive, 2, ',', '.') . "\n";
echo "Quantidade Média de Peças por Sacolinha: " . number_format($qtdMediaItensPorSacola, 1, ',', '.') . " peças\n";
echo "Preço Médio por Peça/Item Vendido: R$ " . number_format($precoMedioPorPeca, 2, ',', '.') . "\n\n";

// 4. Ticket Médio da Live Mais Recente (Live 310)
...
echo "Média de Peças por Cliente na Live #310: " . number_format($itensPorCliente310, 1, ',', '.') . " peças/cliente\n";
