...
(function() {
    ...
    let defaultServerUrl = "https://minhamania.net"; // Servidor de ProduÃ§Ã£o Laravel
    ...
    // FunÃ§Ã£o para realizar requisiÃ§Ãµes via Service Worker em background (evita bloqueios de CSP)
        ...
        return new Promise((resolve, reject) => {
            chrome.runtime.sendMessage({
                ...
                if (response && response.success) {
                    ...
                    reject(new Error(response ? response.error : "Erro de comunicaÃ§Ã£o com o Service Worker"));
                }
            });
        });
    }
    ...
    function detectChatStructure(clickedEl) {
        ...
        // 1. Encontrar o item e contÃªiner com base em repetiÃ§Ã£o de tags de irmÃ£os
        ...
        // 2. Fallback inteligente - encontrar primeiro ancestral estrutural vÃ¡lido (nÃ£o botÃ£o/link/folha)
        const invalidContainerTags = ['button', 'a', 'input', 'textarea', 'p', 'span', 'img', 'svg', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'];
        const invalidContainerRoles = ['button', 'link', 'checkbox', 'menuitem', 'tab'];
    ...
    function findCommentsContainer(root) {
        ...
        // Contar quantos botÃµes de Responder existem no contÃªiner
        ...
        // Se houver botÃµes de resposta, vamos descer a Ã¡rvore atÃ© achar o contÃªiner rolÃ¡vel real
        if (replyCount > 0) {
            let current = root;
            ...
            while (children.length > 0) {
                ...
                for (let child of children) {
                    ...
                    // Se um Ãºnico filho contiver a grande maioria/todos os botÃµes, ele Ã© o contÃªiner real
            ...
            console.log("[Capturador] ContÃªiner de comentÃ¡rios resolvido via contagem de botÃµes:", current);
            return current;
        }
        
        // Fallback: se nÃ£o houver comentÃ¡rios em tela ainda, procurar por um div com id/classe scroll ou scrollview
    ...
    function nameFromAlt(img){
        ...
        var englishName = alt
            .replace(/['â€™]s profile picture.*$/i, "")
            .replace(/['â€™]s profile photo.*$/i, "")
            .trim();
    ...
    function isLikelyProfileImage(img){
        ...
        var alt = (img.alt || "").trim();
        // Emojis tÃªm alt curto sem letras ou nÃºmeros
        if (alt && alt.length <= 2) {
    ...
    // CalibraÃ§Ã£o interativa de seletores inteligente (1 clique)
    calibrateBtn.addEventListener("click", () => {
        alert("Modo de calibraÃ§Ã£o inteligente ativado!\nClique em QUALQUER COMENTÃRIO no chat da live.");
        ...
        overlay.addEventListener("click", (e) => {
            ...
            // Tentar primeiro calibrar usando o contÃªiner detectado automaticamente
            let containerElement = autoDetectContainer();

            // Se a detecÃ§Ã£o automÃ¡tica falhou ou o clique foi fora do contÃªiner
            ...
            alert(`CalibraÃ§Ã£o Inteligente ConcluÃ­da!\n\n` +
                  `1. ContÃªiner detectado: ${selectors.container}\n` +
    ...
    // FunÃ§Ã£o para tratar novas mensagens
    function processNewMessageNode(node) {
        ...
        // Se o prÃ³prio nÃ³ adicionado for o contÃªiner, processar todos os seus filhos
        if (node === chatContainer) {
            Array.from(chatContainer.children).forEach(child => processNewMessageNode(child));
            return;
        }

        // Subir a partir do nÃ³ adicionado atÃ© achar o elemento da mensagem (usando o seletor)
        let rowElement = node;
        let found = false;
        ...
        // Fallback: se nÃ£o encontrou pelo seletor de mensagem, usar o filho direto do contÃªiner
        if (!found) {
            rowElement = node;
        ...
        // Evitar processar o mesmo nÃ³ de comentÃ¡rio mais de uma vez
        if (rowElement.dataset.captured === "true") return;
        rowElement.dataset.captured = "true";

        try {
            // 1. Coletar todas as folhas de texto do nÃ³
            let leafs = [];
            function getLeafTextNodes(el) {
                if (el.nodeType === Node.ELEMENT_NODE) {
                    ...
                    // Ignorar botÃµes, SVGs, menus de aÃ§Ã£o e botÃµes de resposta
            ...
            // Helper para verificar se um texto se parece com badge/nÃ­vel
            function isBadgeText(val) {
                if (!val) return false;
                val = val.trim();
                if (/^\d+$/.test(val)) return true; // nÃºmero puro
            ...
            // Usar o seletor do username configurado se disponÃ­vel
            ...
            // Seletor genÃ©rico fallback para username
            if (!user) {
            ...
            // Se ainda nÃ£o detectamos, pegamos a primeira folha de texto que nÃ£o seja badge
            if (!user) {
            ...
            // Validar username para evitar coletar cabeÃ§alhos de sistema ou ruÃ­dos
            if (!isValidUsername(user)) {
                return;
            }

            // 3. Extrair a mensagem (todos os leafs que vÃªm APÃ“S o username)
            let text = "";
            if (user) {
                let userIdx = -1;
                for (let i = 0; i < leafs.length; i++) {
                ...
                if (userIdx !== -1) {
                    ...
                    for (let i = userIdx + 1; i < leafs.length; i++) {
                        ...
                        // Pular caracteres de separaÃ§Ã£o e botÃµes de controle comuns
                        if (val === ":" || val === "-" || val === "ï¼š" || val === "Â·" || val === "â€¢") continue;
            ...
            console.log(`[Capturador] Tentando processar - UsuÃ¡rio: "${user}", Texto: "${text}"`);
    ...
    function autoDetectContainer() {
        ...
        // 1. HeurÃ­stica de botÃµes "Responder" / "Reply" (especÃ­fica e super estÃ¡vel para Instagram)
        const replyEls = [];
        const candidates = document.querySelectorAll('div, span, button, a');
        for (const el of candidates) {
            if (el.children.length === 0) { // Apenas nÃ³s folhas
        ...
        if (replyEls.length > 0) {
            // Achar o menor ancestral comum que contÃ©m pelo menos 2 botÃµes responder
        ...
        // 2. Fallback: contÃªiner com mÃºltiplos elementos e rolagem (genÃ©rico)
    ...
    function startCapture() {
        ...
        if (platform === "instagram") {
            console.log("[Capturador] Usando estratÃ©gia do Social Stream Ninja + Producer para Instagram (document.body)...");
            ...
            // Observar mudanÃ§as no document.body inteiro
        ...
        // TikTok e fluxo padrÃ£o legados (jÃ¡ funcionais)
        ...
        if (hasCalibrated) {
            ...
            console.log("[Capturador] Usando seletores padrÃ£o para TikTok...");
            chatContainer = document.querySelector(selectors.container);
        } else {
            console.log("[Capturador] Tentando auto-detecÃ§Ã£o inteligente do contÃªiner...");
            const detected = autoDetectContainer();
            if (detected) {
                chatContainer = detected;
                console.log("[Capturador] Auto-detecÃ§Ã£o bem-sucedida!");
            } else {
                console.log("[Capturador] Auto-detecÃ§Ã£o falhou, tentando usar seletor padrÃ£o...");
                chatContainer = document.querySelector(selectors.container);
            }
        }
        ...
        if (chatContainer) {
            ...
            console.log("[Capturador] ContÃªiner definitivo resolvido:", selectors.container);
            console.log("[Capturador] Seletor de mensagens:", selectors.message);
        }

        if (!chatContainer) {
            console.error("[Capturador] ContÃªiner do chat nÃ£o pÃ´de ser localizado pelo seletor:", selectors.container);
            log("ContÃªiner do chat nÃ£o encontrado! Tente 'Calibrar'.", true);
            alert("Aba de chat nÃ£o localizada! Certifique-se de que a live estÃ¡ aberta e o painel de chat visÃ­vel.");
            return;
        }

        console.log("[Capturador] ContÃªiner encontrado:", chatContainer.tagName, chatContainer.className);
        console.log("[Capturador] Seletor de mensagens:", selectors.message);
        ...
        // Processar mensagens que jÃ¡ estÃ£o na tela
