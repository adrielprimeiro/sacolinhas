                ...
                <div class="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach($items as $item)
                        ...
                        <div class="group overflow-hidden rounded-2xl border transition hover:shadow-md
                            {{ $hasPromotion ? 'bg-rose-100 border-rose-200 shadow-sm' : 'bg-white border-zinc-200 shadow-sm' }}">
                            ...
                            <div class="border-t px-4 py-2 {{ $hasPromotion ? 'border-rose-200' : 'border-zinc-100' }}">
                                <div class="mt-2 flex flex-wrap gap-2 text-xs text-zinc-500">
                                    @if($item->cor)
                                        <span class="rounded-full px-2 py-1 {{ $hasPromotion ? 'bg-rose-200' : 'bg-zinc-100' }}">{{ $item->cor }}</span>
                                    @endif

                                    @if($item->tamanho)
                                        <span class="rounded-full px-2 py-1 {{ $hasPromotion ? 'bg-rose-200' : 'bg-zinc-100' }}">{{ $item->tamanho }}</span>
                                    @endif

                                    @if($item->codigo_da_categoria)
                                        <span class="rounded-full px-2 py-1 {{ $hasPromotion ? 'bg-rose-200' : 'bg-zinc-100' }}">Cat: {{ $item->codigo_da_categoria }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
