...
<div class="container">
    <div class="row">
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header">Novo Lançamento de Conta Corrente</div>
                <div class="card-body">
                    <form action="{{ route('admin.financeiro.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" class="form-control @error('data_movimentacao') is-invalid @enderror" id="data_movimentacao" name="data_movimentacao" value="{{ old('data_movimentacao', now()->format('Y-m-d\TH:i')) }}" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('referencia_tipo') is-invalid @enderror" id="referencia_tipo" name="referencia_tipo">
                                <option value="">Nenhum</option> {{-- Adicionado opção "Nenhum" --}}
                                @foreach (['sacolinha', 'pagamento', 'pedido', 'ajuste', 'desconto'] as $refTipo) {{-- Apenas os tipos válidos --}}
                        ...
                        <div class="mb-3" id="referencia_id_group" style="{{ old('referencia_tipo') ? '' : 'display:none;' }}">
                            <label for="referencia_id" class="form-label">ID da Referência</label>
                            <input type="number" class="form-control @error('referencia_id') is-invalid @enderror" id="referencia_id" name="referencia_id" value="{{ old('referencia_id') }}">
                            @error('referencia_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function toggleReferenciaIdField() {
            if (referenciaTipoSelect.value && referenciaTipoSelect.value !== '') { // Mostra se algo foi selecionado
        ...
        // Chamar na carga da página para definir o estado inicial baseado em old()
