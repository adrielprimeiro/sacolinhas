    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 relative overflow-hidden group">
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-black uppercase tracking-widest text-indigo-600 bg-indigo-100 px-3 py-1 rounded-full">{{ $lucratividadePercentual }}% Margem</span>
            </div>
            <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest">Margem de Operação</h3>
            @if($isPositivo)
                <p class="text-3xl font-black mt-1 text-gray-800">R$ {{ number_format($margemBruta, 2, ',', '.') }}</p>
            @else
                <p class="text-3xl font-black mt-1 text-red-600">R$ {{ number_format($margemBruta, 2, ',', '.') }}</p>
            @endif
