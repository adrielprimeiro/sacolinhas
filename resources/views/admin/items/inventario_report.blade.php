...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-500 mt-1">Relatório da distribuição física de peças por prateleira, gaveta ou arara.</p>
        </div>
    ...
    <!-- Tabela de Locais Físicos do Estoque -->
    <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <span class="text-xs text-gray-400 font-medium">Exibindo {{ count($locaisEstoque) }} local(is)</span>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider border-b border-gray-100">
                            ...
                            <th class="py-3.5 px-6 font-semibold">Qtd. de Peças</th>
                            <th class="py-3.5 px-6 font-semibold">Valor Total de Venda</th>
                            <th class="py-3.5 px-6 font-semibold">Valor Médio / Peça</th>
                        ...
                        @foreach($locaisEstoque as $loc)
                            <tr class="hover:bg-indigo-50/40 transition duration-150">
                                <td class="py-4 px-6 font-bold">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-indigo-100 text-indigo-800 font-mono text-sm">
                                        <i class="fas fa-map-pin text-indigo-500 text-xs"></i>
                                        {{ $loc->localizacao }}
                                    </span>
                                </td>
                                <td class="py-4 px-6 font-semibold text-gray-800">
                                    {{ number_format($loc->qtd_pecas, 0, ',', '.') }} peças
                                </td>
                                ...
                                <td class="py-4 px-6 text-gray-600 font-medium">
                                    R$ {{ number_format($loc->valor_medio_venda, 2, ',', '.') }}
                                </td>
                                <td class="py-4 px-6 text-right">
                                    <a href="{{ route('items.index') }}?localizacao={{ urlencode($loc->localizacao) }}" class="inline-flex items-center gap-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-3 py-1.5 rounded-lg text-xs font-semibold transition duration-150 border border-indigo-200">
                                        <i class="fas fa-search"></i>
                                        <span>Ver Peças</span>
                                    </a>
                                </td>
        ...
        @else
            <div class="p-12 text-center text-gray-500">
                ...
                <p class="font-medium text-base">Nenhum local físico encontrado com o filtro atual.</p>
                <p class="text-xs text-gray-400 mt-1">Utilize o Inventário Scanner para bipar e definir localizações físicas para o seu estoque.</p>
                <div class="mt-4">
                    <a href="{{ route('inventario.scanner') }}" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium text-xs px-4 py-2 rounded-lg transition duration-150">
                        <i class="fas fa-qrcode"></i>
                        <span>Abrir Inventário Scanner</span>
                    </a>
                </div>
            </div>
        @endif
