...
async function connectToTikTok(username, liveId) {
    ...
    try {
        ...
        tiktokConnection.on('chat', (data) => {
            const username = (data.user && data.user.displayId) || data.uniqueId || (data.user && data.user.nickname);
            const comment = data.content || data.comment || data.text;
            const avatarUrl = data.profilePictureUrl || null;
            
            if (!username || !comment) {
                console.log("[Chat Debug] Formato desconhecido (faltando username/comment).");
            } else {
                console.log(`[Chat] @${username}: ${comment}`);
                sendToLaravel(username, comment, currentLiveId, avatarUrl);
            }
        });
